...
router.get('/:projectURI', async (req, res) => {
  let projects = await Project.find({ uri: req.params.projectURI })
    .sort({ date: 1 })
    .exec()
  ...
  res.render('project', {
    ...
    dateAxisDataString,
    raiseAxisDataString,
    dailyRaiseDataString,
    backerAxisDataString,
    dailyBackerDataString,
    rankAxisDataString
  })
})

router.get('/:projectURI/widget', async (req, res) => {
  let projects = await Project.find({ uri: req.params.projectURI })
    .sort({ date: 1 })
    .exec()
